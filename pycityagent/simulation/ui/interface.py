...
class InterviewUI:
    ...
    def create_interface(self) -> gr.Blocks:
        ...
        with gr.Blocks(
            ...
            # 设置启动参数
            interface.launch_kwargs = {"quiet": True}
    ...
    def _get_survey_table(self) -> list:
        """获取问卷列表数据"""
    ...
    def _create_survey_center_tab(self):
        ...
        def update_question_editors(q_type):
            """更新问题编辑器的显示状态"""
            is_choice = q_type in [QuestionType.SINGLE_CHOICE.value, QuestionType.MULTIPLE_CHOICE.value]
        ...
        # 在事件绑定之前添加函数定义
        def add_question(q_type, content, options, min_score, max_score, current_questions):
            """添加问题到列表"""
            if not content:
            ...
            if q_type in [QuestionType.SINGLE_CHOICE.value, QuestionType.MULTIPLE_CHOICE.value]:
                # 正确处理 DataFrame 选项，并添加字母标号
                if options is not None and not options.empty:
                    raw_options = [opt[0] for opt in options.values.tolist()]
                    # 为选项添加字母标号 (a, b, c...)
                    question["options"] = [f"{chr(97 + i)}. {opt}" for i, opt in enumerate(raw_options)]
                else:
                    question["options"] = []
    ...
    def _create_survey_tab(self):
        ...
        refresh_btn.click(
            refresh_lists,
            outputs=[agent_select, survey_select]
        )
    ...
    def _create_status_tab(self):
        ...
        refresh_status_btn.click(
            update_status,
            outputs=[total_runtime, status_table, agent_select]
        )
        ...
        block_all_btn.click(block_all, outputs=[total_runtime, status_table, agent_select])
        unblock_all_btn.click(unblock_all, outputs=[total_runtime, status_table, agent_select])
