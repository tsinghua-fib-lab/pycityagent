...
from .survey import SurveyManager, QuestionType
...
class AgentSimulation:
    ...
    def export_chat_history(self, agent_name: str = None) -> str:
        ...
        Returns:
            str: JSON格式的对话历史
        """
        history = self._interview_manager.get_agent_history(agent_name) if agent_name else \
                 self._interview_manager.get_recent_history(limit=1000)
    ...
    async def run(self, steps: int = -1, interval: float = 1.0, start_ui: bool = True, server_name: str = "127.0.0.1", server_port: int = 7860):
        """运行模拟器
        ...
        try:
            ...
            if start_ui:
                ...
                print(f"Gradio Frontend is running on http://{server_name}:{server_port}")
...
"""
            ...
            # 存储原始回答
            self._survey_manager.add_response(survey_id, agent_name, {
    ...
    def create_survey(self, **survey_data: dict) -> list:
        """创建新问卷
        ...
        Returns:
            更新后的问卷列表
        """
        self._survey_manager.create_survey(**survey_data)
