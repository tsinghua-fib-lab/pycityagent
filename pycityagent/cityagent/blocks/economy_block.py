...
class ConsumptionBlock(Block):
    ...
    async def forward(self, step, context):
        ...
        _, post_consumption_wealth = await self.economy_client.calculate_consumption(
            firm_id, [agent_id], [this_demand]
        )
        post_consumption_wealth = post_consumption_wealth[0]
        await self.memory.status.update(
            "consumption_currency",
            consumption_currency + wealth - post_consumption_wealth,
        )
        ...
        await self.memory.status.update(
            "goods_consumption",
            goods_consumption + int((wealth - post_consumption_wealth) // price),
        )
        node_id = await self.memory.stream.add_economy(
            description=f"I bought some goods, and spent {this_demand} on {intention}"
        )
        evaluation = {
            "success": True,
...
class MonthPlanBlock(Block):
    ...
    async def forward(self):
        if await self.month_trigger():
            ...
            await self.memory.status.update(
                "dialog_queue", [{"role": "user", "content": obs_prompt}], mode="merge"
            )
            ...
            await self.economy_client.update(
                agent_id, "currency", wealth + work_hours * skill
            )
            await self.economy_client.add_delta_value(
                firm_id, "inventory", int(work_hours * productivity_per_labor)
            )

            if UBI and self.forward_times >= 96:
                income_currency = await self.memory.status.get("income_currency")
                await self.memory.status.update(
                    "income_currency", income_currency + UBI
                )
                wealth = await self.economy_client.get(agent_id, "currency")
