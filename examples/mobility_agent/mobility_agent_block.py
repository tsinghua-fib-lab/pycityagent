...
from pycityagent.workflow import Block, FormatPrompt
import numpy as np
...
class IntentionBlock(Block):
    ...
    async def forward(self):
        self.prompt.format(**{key: await self.memory.get(key) for key in self.prompt.variables})
...
class MobilityAgent(CitizenAgent):
    def __init__(self, name: str, llm_client: LLM, simulator: Simulator | None = None, memory: Memory | None = None) -> None:
        ...
        self.intentionBlock = IntentionBlock(llm=self.LLM, memory=self.memory, prompt=intention_generation)
    ...
    async def forward(self):
        ...
        for i in range(totalIntention):
            ...
            time, nextPlace = await self.placeBlock.forward(intention, map, nowPlace, time)
