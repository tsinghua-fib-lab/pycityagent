...
def getEventNum():
    ...
    prob = [
        ...
        0.06297312122874382,
        0.03708173340647285,
        0.005156335710367526,
        0.00811848601206802,
        0.0009873834339001646,
        0.0005485463521667581,
        0.0012068019747668679,
        0.0029621503017004938,
        0.0020844761382336806,
        0.00032912781130005483,
        0.0005485463521667581,
        0.00010970927043335162,
    ]
...
def getTimeDistribution():
    ...
    timeDistribution_Event["banking and financial services"] = timeDistribution_Event[
        "handle the trivialities of life"
    ]
    timeDistribution_Event["cultural institutions and events"] = timeDistribution_Event[
        "handle the trivialities of life"
    ]
...
def getBasicData6(personBasicInfo, day, N):
    ...
    educationDescription = (
        "" if education == "uncertain" else "Education: {}; ".format(education)
    )
    consumptionDescription = (
        ""
        if consumption == "uncertain"
        else "Consumption level: {}; ".format(consumption)
    )
    occupationDescription = (
        ""
        if occupation == "uncertain"
        else "Occupation or Job Content: {};".format(occupation)
    )
...
{}{}{}{}""".format(
    ...
    day = (
        ...
        + ". It is important to note that people generally do not work on weekends and prefer entertainment, sports and leisure activities. There will also be more freedom in the allocation of time."
    )
...
def get_poi(sim, id):
    ...
    (res["long"], res["lati"]) = sim.map.xy2lnglat(
    ...
    for poi in pois10k:
        ...
        for d in range(1, 11):
            if (d - 1) * 1000 <= poi[1] < d * 1000:
                ...
                )  # 原来是distanceProb.append(1/(math.sqrt(distance)))
...
def event2poi_gravity(map, label, nowPlace):
    ...
    if nowPlace[1] in map.pois.keys():
        ...
        nowPlace["position"] = {
            "x": nowPlace["positions"][0]["x"],
            "y": nowPlace["positions"][0]["y"],
        }
    labelQueryId = label
