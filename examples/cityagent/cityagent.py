...
class MyAgent(CitizenAgent):
    ...
    async def forward(self):
        ...
        if current_step and current_step.get("type") and current_step.get("intention"):
            ...
            if result != None:
                print(f"执行结果: {result}")
                time_end_exec_actual = str(
                    await self.simulator.get_time(format_time=True)
                )
                time_end_exec_actual = datetime.strptime(
                    time_end_exec_actual, "%H:%M:%S"
                )
                time_end_plan = time_start_exec + timedelta(
                    minutes=result["consumed_time"]
                )
                if time_end_plan >= time_end_exec_actual:
                    ...
                    # 取两者大者
                    result["consumed_time"] = int(
                        (time_end_plan - time_start_exec).total_seconds() / 60
                    )
